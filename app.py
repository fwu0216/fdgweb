...
def sms():
    sms_text = ""
    default_batches = [
        {"name": "第①批", "enabled": True, "dose": 200, "scale_time": "9:00", "arrive_time": "8:10"},
        {"name": "第②批", "enabled": True, "dose": 110, "scale_time": "11:20", "arrive_time": "10:30"},
        {"name": "第③批", "enabled": True, "dose": 30,  "scale_time": "13:20", "arrive_time": "13:00"}
    ]
    ...
    if request.method == "POST":
        ...
        for i in range(1, 4):
            ...
            if enabled:
                batches.append(f"第{i}批需要FDG {dose}mCi，刻度到{scale}\n{arrive}左右送达")
        sms_text = "FDG\n" + "\n".join(batches)
    else:
        batches = default_batches

    return render_template("sms.html", sms_text=sms_text, batches=default_batches)
